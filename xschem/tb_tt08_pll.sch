v {xschem version=3.4.4 file_version=1.2
}
G {}
K {}
V {}
S {}
E {}
B 2 1920 -1450 2720 -1050 {flags=graph
y1=-2.2
y2=7.4
ypos1=0
ypos2=2
divy=5
subdivy=1
unity=1
x1=2e-12
x2=1.2e-05
divx=5
subdivx=1
xlabmag=1.0
ylabmag=1.0


dataset=-1
unitx=1
logx=0
logy=0






color="4 5 17 4"
node="\\"x1.up 4 +\\"
x1.dn
\\"x1.r_low 2 -\\"
\\"VBOOT 0.5 +\\""}
B 2 1920 -1850 2720 -1450 {flags=graph
y1=-0.083
y2=7.9
ypos1=0
ypos2=2
divy=5
subdivy=1
unity=1
x1=2e-12
x2=1.2e-05
divx=5
subdivx=1
xlabmag=1.0
ylabmag=1.0


dataset=-1
unitx=1
logx=0
logy=0








rainbow=0
color="5 4 15 21"
node="\\"VREF 6 +\\"
\\"fb_postdiv 4 +\\"
\\"VREFF 2 +\\"
PLL_OUT"}
B 2 1120 -1050 1920 -650 {flags=graph
y1=-0.047
y2=2
ypos1=0
ypos2=2
divy=5
subdivy=1
unity=1
x1=2e-12
x2=1.2e-05
divx=5
subdivx=1
xlabmag=1.0
ylabmag=1.0


dataset=-1
unitx=1
logx=0
logy=0








color="4 8 5"
node="x1.AB1
x1.x25.A1C
FB_PREDIV"}
B 2 1920 -1050 2720 -650 {flags=graph
y1=0.014
y2=1.8
ypos1=0
ypos2=2
divy=5
subdivy=1
unity=1
x1=2e-12
x2=1.2e-05
divx=5
subdivx=1
xlabmag=1.0
ylabmag=1.0


dataset=-1
unitx=1
logx=0
logy=0






color="18 7"
node="x1.VOUT_CP
x1.VCP"}
B 2 2720 -1450 3520 -1050 {flags=graph
y1=-0.056
y2=2
ypos1=0
ypos2=2
divy=5
subdivy=1
unity=1
x1=2e-12
x2=1.2e-05
divx=5
subdivx=1
xlabmag=1.0
ylabmag=1.0


dataset=-1
unitx=1
logx=0
logy=0








color="4 5 12"
node="x1.UP_LOW
x1.DN_LOW
x1.R_LOW"}
B 2 2720 -1850 3520 -1450 {flags=graph
y1=3.6
y2=12
ypos1=0
ypos2=2
divy=5
subdivy=1
unity=1
x1=2e-12
x2=1.2e-05
divx=5
subdivx=1
xlabmag=1.0
ylabmag=1.0


dataset=-1
unitx=1
logx=0
logy=0






color="4 5 12"
node="\\"x1.UP 8 +\\"
\\"x1.DN 4 +\\"
\\"x1.R 0 +\\""}
B 2 2720 -1050 3520 -650 {flags=graph
y1=0.014
y2=1.8
ypos1=0
ypos2=2
divy=5
subdivy=1
unity=1
x1=2e-12
x2=1.2e-05
divx=5
subdivx=1
xlabmag=1.0
ylabmag=1.0


dataset=-1
unitx=1
logx=0
logy=0






color=18
node=x1.vcp}
B 2 2720 -2250 3520 -1850 {flags=graph
y1=-0.091
y2=10
ypos1=0
ypos2=2
divy=5
subdivy=1
unity=1
x1=2e-12
x2=1.2e-05
divx=5
subdivx=1
xlabmag=1.0
ylabmag=1.0


dataset=-1
unitx=1
logx=0
logy=0







color="5 4 8 10 12"
node="B1 
\\"D1 2 +\\"
\\"D2 4 +\\"
\\"D3 6 +\\"
\\"C1 8 +\\""}
T {Current source reference} 750 -1590 0 0 0.4 0.4 {}
N 1940 -2020 1940 -2010 {
lab=VSS}
N 2050 -2020 2050 -2010 {
lab=#net1}
N 1940 -1950 1940 -1940 {
lab=GND}
N 2050 -1950 2050 -1940 {
lab=GND}
N 2050 -2120 2050 -2100 {
lab=VDD}
N 2050 -2040 2050 -2020 {
lab=#net1}
N 2330 -1970 2330 -1920 {
lab=VSS}
N 2270 -2020 2270 -2010 {
lab=VDPWR}
N 2270 -1950 2270 -1940 {
lab=GND}
N 2270 -2040 2270 -2020 {
lab=VDPWR}
N 1860 -2010 1860 -2000 {
lab=VAPWR}
N 1860 -1940 1860 -1930 {
lab=GND}
N 1860 -2030 1860 -2010 {
lab=VAPWR}
N 1070 -1150 1070 -1080 {
lab=#net2}
N 1010 -1080 1070 -1080 {
lab=#net2}
N 1070 -1330 1070 -1150 {
lab=#net2}
N 1010 -1130 1070 -1130 {
lab=#net2}
N 980 -990 980 -960 {
lab=VAPWR}
N 940 -930 940 -900 {
lab=#net3}
N 940 -900 980 -900 {
lab=#net3}
N 870 -990 870 -960 {
lab=VAPWR}
N 870 -990 980 -990 {
lab=VAPWR}
N 910 -930 940 -930 {
lab=#net3}
N 1010 -1150 1090 -1170 {
lab=#net4}
N 1090 -1170 1090 -840 {
lab=#net4}
N 980 -840 1090 -840 {
lab=#net4}
N 730 -990 730 -960 {
lab=VAPWR}
N 730 -990 870 -990 {
lab=VAPWR}
N 770 -930 910 -930 {
lab=#net3}
N 770 -760 820 -760 {
lab=VBIASN_G5W2L1}
N 730 -730 730 -700 {
lab=VSS}
N 730 -840 730 -790 {
lab=VBIASN_G5W2L1}
N 730 -810 780 -810 {
lab=VBIASN_G5W2L1}
N 780 -810 780 -760 {
lab=VBIASN_G5W2L1}
C {devices/code.sym} 2490 -2030 0 0 {name=TT_MM_MODELS
only_toplevel=true
format="tcleval( @value )"
value="
** opencircuitdesign pdks install
.lib $::SKYWATER_MODELS/sky130.lib.spice tt
.include $::SKYWATER_STDCELLS/sky130_fd_sc_hd.spice                                                              
                        
"
spice_ignore=false}
C {devices/launcher.sym} 1880 -2080 0 0 {name=h17 
descr="Load waves" 
tclcommand="
xschem raw_read $netlist_dir/[file tail [file rootname [xschem get current_name]]].raw tran

"
}
C {devices/simulator_commands_shown.sym} 680 -2200 0 0 {name=COMMANDS2
simulator=ngspice
only_toplevel=false 
value="
.options savecurrents 
.OPTIONS RAMPTIME=1ns
VREF VREF 0 pulse 1.8 0 100n 10p 10p 8n 16n
VREFF VREFF 0 pulse 1.8 0 100n 50p 50p 0.5n 1n
VBOOT VBOOT 0 pulse 1.8 0 0n 2n 2n 200n 200u
.control
  repeat 1
    save all
    tran 200p 12u uic
    remzerovec
    write tb_tt08_pll.raw
    set appendwrite
    reset
    end
.endc
"}
C {devices/vsource.sym} 2050 -1980 0 0 {name=V1 value=1.8 savecurrent=false}
C {devices/gnd.sym} 2050 -1940 0 0 {name=l2 lab=GND}
C {devices/lab_pin.sym} 2050 -2120 2 1 {name=p19 sig_type=std_logic lab=VDD
}
C {devices/vsource.sym} 1940 -1980 0 0 {name=V2 value=0 savecurrent=false}
C {devices/gnd.sym} 1940 -1940 0 0 {name=l3 lab=GND}
C {devices/lab_pin.sym} 1940 -2020 2 1 {name=p28 sig_type=std_logic lab=VSS
}
C {devices/ammeter.sym} 2050 -2070 2 1 {name=vdd_totpw savecurrent=true}
C {devices/ammeter.sym} 2330 -2000 2 1 {name=Vmeasn1 savecurrent=true}
C {devices/lab_pin.sym} 2330 -2030 2 0 {name=p12 sig_type=std_logic lab=VGND
}
C {devices/lab_pin.sym} 2330 -1930 2 0 {name=p26 sig_type=std_logic lab=VSS
}
C {devices/vsource.sym} 2270 -1980 0 0 {name=V4 value=1.8 savecurrent=false}
C {devices/gnd.sym} 2270 -1940 0 0 {name=l1 lab=GND}
C {devices/lab_pin.sym} 2270 -2040 2 1 {name=p15 sig_type=std_logic lab=VDPWR

}
C {devices/vsource.sym} 1860 -1970 0 0 {name=V7 value=3.3 savecurrent=false}
C {devices/gnd.sym} 1860 -1930 0 0 {name=l4 lab=GND}
C {devices/lab_pin.sym} 1860 -2030 2 1 {name=p5 sig_type=std_logic lab=VAPWR

}
C {devices/lab_pin.sym} 1380 -1690 2 1 {name=p27 sig_type=std_logic lab=VBOOT
}
C {root_currents1.sym} 860 -1490 0 0 {name=x2}
C {/home/emilian/tt/local/share/xschem/xschem_library/devices/lab_pin.sym} 1010 -1490 2 0 {name=p34 sig_type=std_logic lab=VBIASP}
C {/home/emilian/tt/local/share/xschem/xschem_library/devices/gnd.sym} 930 -1440 0 0 {name=l7 lab=GND}
C {/home/emilian/tt/local/share/xschem/xschem_library/devices/lab_pin.sym} 800 -1550 0 0 {name=p35 sig_type=std_logic lab=VDPWR}
C {/home/emilian/tt/local/share/xschem/xschem_library/devices/lab_pin.sym} 710 -1520 0 0 {name=p36 sig_type=std_logic lab=VDD}
C {devices/lab_pin.sym} 710 -1470 0 0 {name=p38 sig_type=std_logic lab=VSS}
C {devices/lab_pin.sym} 900 -1550 2 1 {name=p33 sig_type=std_logic lab=VAPWR

}
C {root_currents1outp.sym} 860 -1330 0 0 {name=x9}
C {devices/ammeter.sym} 1040 -1330 3 0 {name=viout2 savecurrent=true}
C {/home/emilian/tt/local/share/xschem/xschem_library/devices/lab_pin.sym} 710 -1330 0 0 {name=p46 sig_type=std_logic lab=VBIASP}
C {cmirror_adj1.sym} 860 -1090 0 0 {name=x16}
C {/home/emilian/tt/local/share/xschem/xschem_library/devices/lab_pin.sym} 860 -1040 0 0 {name=p66 sig_type=std_logic lab=VSS}
C {devices/lab_pin.sym} 710 -1150 0 0 {name=p52 sig_type=std_logic lab=VSS}
C {devices/lab_pin.sym} 710 -1130 0 0 {name=p53 sig_type=std_logic lab=VSS}
C {devices/lab_pin.sym} 710 -1110 0 0 {name=p56 sig_type=std_logic lab=VSS}
C {devices/lab_pin.sym} 800 -1430 0 0 {name=p69 sig_type=std_logic lab=VSS}
C {devices/ammeter.sym} 980 -870 0 1 {name=viref savecurrent=true}
C {sky130_fd_pr/pfet3_g5v0d10v5.sym} 960 -930 0 0 {name=M4
W=4
L=1
body=VDDCP
nf=1
mult=1
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_g5v0d10v5
spiceprefix=X
}
C {devices/ammeter.sym} 870 -870 0 0 {name=virefcp savecurrent=true}
C {sky130_fd_pr/pfet3_g5v0d10v5.sym} 890 -930 0 1 {name=M1
W=4
L=1
body=VDDCP
nf=1
mult=1
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_g5v0d10v5
spiceprefix=X
}
C {devices/lab_pin.sym} 870 -840 2 0 {name=p68 sig_type=std_logic lab=IREF_CP}
C {devices/ammeter.sym} 730 -870 0 0 {name=virefopamp savecurrent=true}
C {sky130_fd_pr/pfet3_g5v0d10v5.sym} 750 -930 0 1 {name=M2
W=4
L=1
body=VDDCP
nf=1
mult=1
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_g5v0d10v5
spiceprefix=X
}
C {devices/lab_pin.sym} 810 -760 2 0 {name=p50 sig_type=std_logic lab=VBIASN_G5W2L1}
C {sky130_fd_pr/nfet_g5v0d10v5.sym} 750 -760 0 1 {name=M7
W=2
L=1
nf=1
mult=1
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=nfet_g5v0d10v5
spiceprefix=X
}
C {/home/emilian/tt/local/share/xschem/xschem_library/devices/lab_pin.sym} 730 -700 0 1 {name=p73 sig_type=std_logic lab=VSS}
C {devices/lab_pin.sym} 710 -1090 0 0 {name=p51 sig_type=std_logic lab=VSS}
C {devices/lab_pin.sym} 1380 -1600 0 0 {name=p78 sig_type=std_logic lab=VBIASN_G5W2L1}
C {tt08_pll.sym} 1530 -1640 0 0 {name=x1}
C {devices/lab_pin.sym} 1380 -1580 0 0 {name=p1 sig_type=std_logic lab=IREF_CP}
C {devices/lab_pin.sym} 1530 -1540 2 0 {name=p2 sig_type=std_logic lab=VSS
}
C {devices/lab_pin.sym} 1520 -1740 3 1 {name=p3 sig_type=std_logic lab=VAPWR

}
C {devices/lab_pin.sym} 1540 -1740 3 1 {name=p4 sig_type=std_logic lab=VDPWR

}
C {devices/lab_pin.sym} 1680 -1710 2 0 {name=p6 sig_type=std_logic lab=PLL_OUT

}
C {devices/lab_pin.sym} 1680 -1610 2 0 {name=p7 sig_type=std_logic lab=FB_PREDIV

}
C {devices/lab_pin.sym} 1680 -1590 2 0 {name=p8 sig_type=std_logic lab=FB_POSTDIV

}
C {devices/lab_pin.sym} 1380 -1710 2 1 {name=p9 sig_type=std_logic lab=VREF
}
C {devices/lab_pin.sym} 860 -1390 2 1 {name=p10 sig_type=std_logic lab=VAPWR

}
C {/home/emilian/tt/local/share/xschem/xschem_library/devices/lab_pin.sym} 860 -1180 0 0 {name=p11 sig_type=std_logic lab=VDPWR}
C {devices/lab_pin.sym} 980 -990 2 0 {name=p13 sig_type=std_logic lab=VAPWR

}
